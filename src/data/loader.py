...
class LenMatchBatchSampler(BatchSampler):
    ...
    The `LenMatchBatchSampler` is designed to work with data loaders where the underlying dataset
    provides the sequence lengths. It uses the sequence lengths to group samples into buckets based
    on their lengths. Batches are then generated by sampling samples from each bucket to ensure that
    they have similar sequence lengths.
