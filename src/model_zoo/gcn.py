...
from model_zoo.graph_utils import SpatialGraph
...
class DecoupledGCN(nn.Module):
    ...
    def __init__(
        ...
        num_points = n_landmarks
        
        edges = [[(g[i], g[i + 1]) for i in range(len(g) - 1)] for g in GRAPH]
        edges = np.concatenate(edges)
        inward_edges = [[i, j] for i in range(n_landmarks) for j in range(n_landmarks) if i < j]
...
#         print(inward_edges)
        ...
        self.l2 = DecoupledGCN_TCN_unit(
            dim // 4, dim // 2, A, groups, num_points, block_size, drop_size=drop_size
        )
        self.l3 = DecoupledGCN_TCN_unit(
            dim // 2, dim, A, groups, num_points, block_size, drop_size=drop_size
        )
        self.l4 = DecoupledGCN_TCN_unit(
            dim, dim, A, groups, num_points, block_size, drop_size=drop_size
        )
#         self.l5 = DecoupledGCN_TCN_unit(
#             dim // 4, dim // 2, A, groups, num_points, block_size, drop_size=drop_size, stride=2
#         )
#         self.l6 = DecoupledGCN_TCN_unit(
#             dim // 2, dim // 2, A, groups, num_points, block_size, drop_size=drop_size
#         )
#         self.l7 = DecoupledGCN_TCN_unit(
#             dim // 2, dim // 2, A, groups, num_points, block_size, drop_size=drop_size
#         )
#         self.l8 = DecoupledGCN_TCN_unit(
#             dim // 2, dim, A, groups, num_points, block_size, drop_size=drop_size, stride=2
#         )
...
#         )
    ...
    def forward(self, x, keep_prob=1.):
        ...
        """
        N, C, T, V = x.size()  # bs x n_fts x n_frames x n_nodes
        x = x.permute(0, 3, 1, 2).contiguous().view(N, V * C, T)
        x = self.data_bn(x)
        x = x.view(N, V, C, T).permute(0, 2, 3, 1).contiguous()  # bs x n_fts x n_frames x n_nodes
...
#         x = self.l10(x, keep_prob)
        ...
        return x  # bs x n_fts x n_frames x n_nodes
