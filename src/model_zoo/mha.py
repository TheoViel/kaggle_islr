...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = F.linear(
            x[:, :1],
            self.mha.in_proj_weight[: self.embed_dim],
            self.mha.in_proj_bias[: self.embed_dim],
        )  # since we need only cls
        k = F.linear(
            x,
            self.mha.in_proj_weight[self.embed_dim: self.embed_dim * 2],
            self.mha.in_proj_bias[self.embed_dim: self.embed_dim * 2],
        )
        v = F.linear(
            x,
            self.mha.in_proj_weight[self.embed_dim * 2:],
            self.mha.in_proj_bias[self.embed_dim * 2:],
        )

        #         print(q.size(), k.size(), v.size())

        q = q.reshape(bs, -1, self.num_head, self.embed_dim // self.num_head).permute(
            0, 2, 1, 3
        )  # BS x n_heads x 1 x n_fts
        k = k.reshape(bs, -1, self.num_head, self.embed_dim // self.num_head).permute(
            0, 2, 3, 1
        )  # BS x n_heads x n_fts x landmarks
        v = v.reshape(bs, -1, self.num_head, self.embed_dim // self.num_head).permute(
            0, 2, 1, 3
        )  # BS x n_heads x landmarks x n_fts
        ...
        out = F.linear(
            out, self.mha.out_proj.weight, self.mha.out_proj.bias
        )  # BS x n_fts*n_heads
