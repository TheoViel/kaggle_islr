...
class DisentangledSelfAttention(nn.Module):
    ...
    Parameters:
        config (`DebertaV2Config`):
            ...
            The schema is similar to *BertConfig*, for more details please refer [`DebertaV2Config`]
    """
    ...
    def transpose_for_scores(self, x, attention_heads):
        ...
        return (
            x.permute(0, 2, 1, 3)
            .contiguous()
            .view(-1, nobuco.shape(x)[1], nobuco.shape(x)[-1])
        )
    ...
    def forward(
        ...
        Args:
            hidden_states (`torch.FloatTensor`):
                Input states to the module usually the output from previous layer,
                it will be the Q, K and V in *Attention(Q,K,V)*
            attention_mask (`torch.ByteTensor`):
                An attention mask matrix of shape [*B*, *N*, *N*] where *B* is the batch size,
                *N* is the maximum sequence length in which element [i,j] = *1* means
            ...
            relative_pos (`torch.LongTensor`):
                ...
                It's of shape [*B*, *N*, *N*] with values ranging
                in [*-max_relative_positions*, *max_relative_positions*].
    ...
    def efficient_disentangled_attention_bias(
        ...
        if "p2c" in self.pos_att_type:  # position->content
            ...
            p2c_att = (
                self.selector(p2c_att.view(-1), ids_t)
                .view(bs, sz, sz)
                .transpose(-1, -2)
            )
