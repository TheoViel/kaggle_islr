...
def load_model_weights(model, filename, verbose=1, cp_folder="", strict=True):
    ...
    try:  # Handle DDP
    ...
    except BaseException:  # Handle last layer 
        ...
        try:  # REMOVE CLASSIFIER
            ...
            del state_dict_["logits.weight"], state_dict_["logits.bias"]
            model.load_state_dict(state_dict_, strict=False)

    if verbose:
        print(
            f"\n -> Loading model weights from {os.path.join(cp_folder, filename)}\n"
        )

    return model


def load_encoder(model, filename, verbose=1, cp_folder="", strict=True):
    """
    Loads the weights of a PyTorch model. The exception handles cpu/gpu incompatibilities.
    ...
    for k, v in state_dict.items():
        if k.startswith("encoder."):
            state_dict_[re.sub("encoder.", "", k)] = v

    model.encoder.load_state_dict(state_dict_, strict=True)
